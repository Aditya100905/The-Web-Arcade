...
import { Zap, Hand, FileText, Scissors, RotateCcw, Trophy, Target } from "lucide-react";
import { GiStoneBlock  } from 'react-icons/gi';

function MyComponent() {
  return (
    <div>
      <h3>Have a <GiStoneBlock  /> on me!</h3>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900 text-white flex flex-col items-center justify-center p-4 sm:p-6">
      ...
      <div className="text-center mb-8 sm:mb-12">
        ...
        <p className="text-sm sm:text-base text-gray-300">Battle of Strategy & Luck</p>
      </div>
      ...
      <div className="w-full max-w-4xl mx-auto">
        ...
        {!showResult && !isAnimating ? (
          ...
          <div className="text-center py-8 sm:py-16">
            <div className="inline-flex items-center gap-3 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-full px-6 sm:px-8 py-3 sm:py-4 shadow-2xl">
              ...
              <p className="text-lg sm:text-xl font-semibold">Computer is thinking...</p>
            </div>
          </div>
        ) : (
          <div className={`result-container transition-all duration-500 ${showResult ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl ${
              gameResult === 'win' ? 'bg-gradient-to-br from-green-600 to-emerald-600' :
              gameResult === 'lose' ? 'bg-gradient-to-br from-red-600 to-rose-600' :
              'bg-gradient-to-br from-yellow-600 to-amber-600'
            }`}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-12 items-center">
                <div className="text-center">
                  <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">Your Choice</h3>
                  <div className="bg-white/20 rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
                    {userChoice && React.createElement(getChoiceIcon(userChoice), {
                      className: "w-16 h-16 sm:w-20 sm:h-20 mx-auto text-white"
                    })}
                  </div>
                </div>
                <div className="text-center">
                  <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">Computer's Choice</h3>
                  <div className="bg-white/20 rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
                    {compChoice && React.createElement(getChoiceIcon(compChoice), {
                      className: "w-16 h-16 sm:w-20 sm:h-20 mx-auto text-white"
                    })}
                  </div>
                </div>
        ...
        <div className="text-center mt-8 sm:mt-12">
          <div className={`inline-block px-4 sm:px-6 py-3 sm:py-4 rounded-2xl text-base sm:text-xl font-semibold shadow-2xl transition-all duration-300 ${
            gameResult === 'win' ? 'bg-gradient-to-r from-green-500 to-emerald-500 animate-pulse' :
            gameResult === 'lose' ? 'bg-gradient-to-r from-red-500 to-rose-500' :
            gameResult === 'draw' ? 'bg-gradient-to-r from-yellow-500 to-amber-500' :
            'bg-gradient-to-r from-indigo-600 to-purple-600'
          }`}>
            {gameResult === 'win' && <Trophy className="inline w-5 h-5 sm:w-6 sm:h-6 mr-2" />}
            {message}
          </div>
        </div>
