...
const FlappyBird = () => {
  ...
  const PIPE_GAP =
    gameMode === "hard"
      ? Math.max(120, dimensions.height * 0.18)
      : gameMode === "easy"
  ...
  const jump = useCallback(() => {
    if (gameState === "waitingForStart") {
      ...
      createParticle(
        dimensions.width * 0.15 + BIRD_SIZE / 2,
        birdY + BIRD_SIZE / 2,
        "jump"
      );
    }
  ...
  useEffect(() => {
    ...
    gameLoopRef.current = setInterval(() => {
      setBirdTrail((prev) => [
        ...prev.slice(-8),
        { x: dimensions.width * 0.15, y: birdY, time: Date.now() },
      ]);
      ...
      setPipes((prev) => {
        ...
        if (
          newPipes.length === 0 ||
          newPipes[newPipes.length - 1].x < dimensions.width - 250
        ) {
          const minGapY = 80;
      ...
      setPipes((currentPipes) => {
        ...
        for (let pipe of currentPipes) {
          const tolerance = 2;
          if (
            birdRight > pipe.x + tolerance &&
            birdLeft < pipe.x + PIPE_WIDTH - tolerance
          ) {
            if (
              birdTop < pipe.topHeight - tolerance ||
              birdBottom > pipe.bottomY + tolerance
            ) {
              gameOver();
              return currentPipes;
            }
    ...
    <div className="absolute inset-0 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center z-30 p-4">
      <div className="text-center bg-gradient-to-br from-indigo-900/90 via-purple-900/90 to-violet-900/90 p-6 md:p-8 rounded-2xl border border-purple-400/30 shadow-2xl backdrop-blur-sm w-full max-w-md">
        ...
        <div className="text-xs md:text-sm text-purple-300 bg-black/20 p-3 md:p-4 rounded-xl">
          <div className="mb-3 font-semibold text-yellow-400">
            🏆 Best Scores
          </div>
        ...
        <div className="space-y-3 md:space-y-4 mb-4 md:mb-6">
          {Object.entries(modes).map(([key, mode]) => (
            ...
            >
              ...
              <div className="text-xs md:text-sm opacity-90 ml-8 md:ml-11">
                {mode.desc}
              </div>
  ...
  const getPipeDesign = (type, isTop, width, height) => {
    ...
    return (
      <div
        className={`w-full h-full ${design.main} ${design.border} border-r-2 shadow-lg relative`}
      >
        <div
          className={`absolute ${isTop ? "bottom-0" : "top-0"} left-0 right-0 ${
            design.cap
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 text-white overflow-hidden">
      <div
        className="container mx-auto px-4 py-4 flex flex-col items-center min-h-screen"
        ref={containerRef}
      >
              ...
              <div key={pipe.id}>
                ...
                >
                  {getPipeDesign(
                    ...
                    false,
                    PIPE_WIDTH,
                    dimensions.height - pipe.bottomY
                  )}
                </div>
              </div>
            ))}
